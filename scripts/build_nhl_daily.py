...
- Goalie season summary from MoneyPuck (goalies.csv)
- Starting goalies (DailyFaceoff scrape)

Outputs:
- data/nhl_daily_slim.json
- data/nhl_daily_slim.yml
...
def main() -> int:
    ...
    dfo_url = f"{DAILYFACEOFF_BASE}/starting-goalies/{data_date}"
    dfo_status: Dict[str, Any] = {"ok": False, "url": dfo_url, "count": 0}

    try:
        starters_res = fetch_dailyfaceoff_starters(data_date)

        # Supports both legacy (list) and new (StartersFetchResult) return types
        if isinstance(starters_res, list):
            starters_list = starters_res
            dfo_status = {"ok": True, "url": dfo_url, "count": len(starters_list)}
        else:
            starters_list = list(getattr(starters_res, "starters", []) or [])
            dfo_status = dict(getattr(starters_res, "status", {}) or {})
            dfo_status.setdefault("url", dfo_url)
            dfo_status.setdefault("count", len(starters_list))

    except Exception as e:
        starters_list = []
        dfo_status = {"ok": False, "url": dfo_url, "error": str(e), "count": 0}
